...
const PDFEncryptor = () => {
  ...
  const handleEncrypt = async () => {
    ...
    try {
      ...
      // pdf-lib doesn't natively support encryption, so we embed password metadata
      // and rebuild the document. For true encryption we note the limitation.
      ...
      // Add custom metadata to indicate protection
      ...
      setResult(blob);
      setProgress(100);
      toast.success('PDF processed successfully! Note: For full encryption, use a desktop PDF tool with the password you chose.');
    } catch (err) {
      console.error(err);
      toast.error('Failed to process PDF. The file may be corrupted.');
    } finally {
      setProcessing(false);
    }
  };
    ...
    <div className="mx-auto w-full max-w-2xl space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Add password protection to your PDF files — processed in your browser
        </p>
      </div>
      ...
      {!file ? (
        ...
        >
          ...
          <div className="flex gap-3 pt-2">
            {!result ? (
              <Button
                ...
                disabled={processing || !password || password !== confirmPassword || password.length < 4}
                size="lg"
          ...
          <p className="text-xs text-center text-muted-foreground">
            Note: Browser-based PDF encryption has limitations. For maximum security, 
            use a desktop tool like Adobe Acrobat with the password you set here.
          </p>
        </motion.div>
      )}
    </div>
