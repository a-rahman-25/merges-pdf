...
serve(async (req) => {
  ...
  try {
    ...
    const systemContent = hasText
      ? `You are a friendly, conversational PDF assistant. The user uploaded "${filename}" (${pageCount} pages). Here is the text:\n\n${textContent}\n\nEngage in natural conversation about this document. Answer questions accurately, cite specific parts, suggest follow-ups, and be helpful. Use markdown formatting for clarity.`
      : `You are a friendly PDF assistant. The user uploaded "${filename}" (${pageCount} pages). Help them with questions about the document. Be conversational and helpful. Use markdown formatting.`;
