...
const PDFSplitter = () => {
  ...
  const handleSplit = async () => {
    ...
    if (splitMode === 'all') {
      if (file.pageCount !== null && file.pageCount < 2) {
        toast.error('This PDF has only one page — nothing to split.');
        return;
      }
      setSplitting(true);
      try {
      ...
      // Range mode — extract as single PDF
      ...
      try {
        ...
        downloadBlob(data, `${baseName}_pages_${rangeInput.replace(/\s/g, '')}.pdf`);
        setSplitDone(true);
  ...
  const downloadPage = (page: { data: Uint8Array; name: string }) => {
    downloadBlob(page.data, page.name);
  };

  const downloadAll = () => {
    results.forEach((page) => downloadBlob(page.data, page.name));
  };
                ...
                {splitMode === 'range' && (
                  ...
                  >
                    ...
                    <p className="text-xs text-muted-foreground text-center">
                      Pages will be extracted into a single PDF
                    </p>
                  </motion.div>
                )}
              </div>
                ...
                >
                  {splitting ? (
                    <>
                      ...
                      {splitMode === 'range' ? 'Extracting…' : 'Splitting…'}
                    </>
                  ) : (
                    <>
                      <Scissors className="h-5 w-5" />
                      {splitMode === 'range' ? 'Extract Pages' : 'Split into Pages'}
                    </>
                  )}
                </Button>
              </motion.div>
              ...
              >
                <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                  {results.map((page, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-3 rounded-xl bg-card p-3 pr-4 border border-border"
                    >
