...
import { Braces, Loader2, RotateCcw, Copy, Check } from 'lucide-react';
...
const PDFToJSON = () => {
  ...
  const doDownload = useCallback((filename?: string) => {
    ...
    saveAs(result.blob, filename || `${baseName}.json`);
    ...
    try {
      ...
      const jsonData = {
        ...
        pages: pages.map((pageText, i) => {
          const dims = pdf.getPage(i);
          return {
            page: i + 1,
            width: Math.round(dims.getWidth()),
            height: Math.round(dims.getHeight()),
            text: pageText.trim(),
          };
        }),
      };
      ...
      {!file ? (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} onClick={() => inputRef.current?.click()}
          ...
          <div className="flex flex-col items-center gap-4">
            <div className="rounded-xl bg-primary/10 p-4"><Braces className="h-8 w-8 text-primary" /></div>
            <div>
              <p className="text-lg font-display font-semibold text-foreground">Select a PDF to convert</p>
              <p className="mt-1 text-sm text-muted-foreground">Extract PDF content as structured JSON data</p>
            </div>
          </div>
        </motion.div>
      ) : !result ? (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          ...
          <Button onClick={handleConvert} disabled={processing} size="lg" className="w-full gap-2 text-base font-display font-semibold h-14 rounded-xl">
            {processing ? (<><Loader2 className="h-5 w-5 animate-spin" />Converting…</>) : (<><Braces className="h-5 w-5" />Convert to JSON</>)}
          </Button>
        </motion.div>
      ) : (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          <PreDownloadSummary
            ...
            items={[
              ...
              { label: 'Output', value: file.name.replace(/\.pdf$/i, '.json') },
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            ...
            <pre className="text-xs text-muted-foreground whitespace-pre-wrap max-h-48 overflow-y-auto font-mono">{result.preview}{result.preview.length >= 1200 ? '\n...' : ''}</pre>
          </div>
        </motion.div>
      )}
      <ReviewDialog open={showReview} toolName="PDF to JSON" onSubmit={handleSubmit} onSkip={handleSkip} />
    </div>
  );
};
