      ...
      {result ? (
        <motion.div key="result" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
          <div className="rounded-xl border border-border bg-card p-6 text-center">
            <p className="text-lg font-semibold text-foreground">Pages Reordered!</p>
            <p className="mt-1 text-sm text-muted-foreground">{pages.length} pages · {formatFileSize(result.data.length)}</p>
          </div>
          <div className="flex justify-center gap-3">
            <Button onClick={() => { const baseName = file!.name.replace(/\.pdf$/i, ''); downloadBlob(result.data, `${baseName}_reordered.pdf`); }} size="lg" className="rounded-xl px-8">Download PDF</Button>
